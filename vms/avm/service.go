...
func (service *Service) GetBalance(r *http.Request, args *GetBalanceArgs, reply *GetBalanceReply) error {
	...
	for _, utxo := range utxos {
		...
		transferable, ok := utxo.Out.(djtx.TransferableOut)
		if !ok {
			continue
		}
...
func (service *Service) CreateFixedCapAsset(r *http.Request, args *CreateFixedCapAssetArgs, reply *FormattedAssetID) error {
	...
	djtxKey := service.vm.ctx.DJTXAssetID.Key()
	amountsSpent, ins, keys, err := service.vm.Spend(
		utxos,
		kc,
		map[[32]byte]uint64{
			djtxKey: service.vm.txFee,
		},
	)
	if err != nil {
		return err
	...
	if amountSpent := amountsSpent[djtxKey]; amountSpent > service.vm.txFee {
		changeAddr := kc.Keys[0].PublicKey().Address()
		outs = append(outs, &djtx.TransferableOutput{
			Asset: djtx.Asset{ID: service.vm.ctx.DJTXAssetID},
...
func (service *Service) Send(r *http.Request, args *SendArgs, reply *api.JsonTxID) error {
	...
	djtxKey := service.vm.ctx.DJTXAssetID.Key()
	amountWithFee, err := safemath.Add64(amountsWithFee[djtxKey], service.vm.txFee)
	if err != nil {
		return fmt.Errorf("problem calculating required spend amount: %w", err)
	}
	amountsWithFee[djtxKey] = amountWithFee
...
// ImportDJTXArgs are arguments for passing into ImportDJTX requests
...
// ImportDJTX imports DJTX to this chain from the P-Chain.
// The DJTX must have already been exported from the P-Chain.
// Returns the ID of the newly created atomic transaction
func (service *Service) ImportDJTX(_ *http.Request, args *ImportDJTXArgs, reply *api.JsonTxID) error {
	service.vm.ctx.Log.Info("AVM: ImportDJTX called with username: %s", args.Username)
	...
	if amountSpent := amountsSpent[djtxKey]; amountSpent < service.vm.txFee {
		...
		localAmountsSpent, ins, keys, err = service.vm.Spend(
			...
			kc,
			map[[32]byte]uint64{
				djtxKey: service.vm.txFee - amountSpent,
			},
		)
		if err != nil {
			return err
	...
	amountsSpent[djtxKey] -= service.vm.txFee
...
type ExportDJTXArgs struct {
	api.UserPass // User providing exported DJTX
...
// ExportDJTX sends DJTX from this chain to the P-Chain.
// After this tx is accepted, the DJTX must be imported to the P-chain with an importTx.
// Returns the ID of the newly created atomic transaction
func (service *Service) ExportDJTX(_ *http.Request, args *ExportDJTXArgs, reply *api.JsonTxID) error {
	service.vm.ctx.Log.Info("AVM: ExportDJTX called with username: %s", args.Username)
	...
	amountSpent := amountsSpent[djtxKey]
