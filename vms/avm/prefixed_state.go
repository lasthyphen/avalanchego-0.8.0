...
func (s *prefixedState) UTXO(id ids.ID) (*djtx.UTXO, error) {
	return s.state.UTXO(uniqueID(id, utxoID, s.utxo))
}

// SetUTXO saves the provided utxo to storage.
func (s *prefixedState) SetUTXO(id ids.ID, utxo *djtx.UTXO) error {
...
func (s *prefixedState) FundUTXO(utxo *djtx.UTXO) error {
	utxoID := utxo.InputID()
