...
func TestServiceGetUTXOs(t *testing.T) {
	...
	for i := 0; i < numUTXOs; i++ {
		if err := vm.state.FundUTXO(&djtx.UTXO{
			UTXOID: djtx.UTXOID{
...
func TestImportDJTX(t *testing.T) {
	...
	// Must set DJTX assetID to be the correct asset since only DJTX can be imported
	utxo := &djtx.UTXO{
	...
	reply := &api.JsonTxID{}
	if err := s.ImportDJTX(nil, args, reply); err != nil {
