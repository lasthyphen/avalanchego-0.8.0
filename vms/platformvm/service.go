...
func (service *Service) ExportDJTX(_ *http.Request, args *ExportDJTXArgs, response *api.JsonTxID) error {
	service.vm.Ctx.Log.Info("Platform: ExportDJTX called")
...
func (service *Service) ImportDJTX(_ *http.Request, args *ImportDJTXArgs, response *api.JsonTxID) error {
	service.vm.Ctx.Log.Info("Platform: ImportDJTX called")
...
func (service *Service) GetStake(_ *http.Request, args *api.JsonAddresses, response *GetStakeReply) error {
	...
	helper := func(tx *Tx) (uint64, error) {
		...
		for _, stake := range outs {
			if !stake.AssetID().Equals(service.vm.Ctx.DJTXAssetID) {
				continue
			}
