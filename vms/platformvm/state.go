...
func (vm *VM) getUTXO(db database.Database, ID ids.ID) (*djtx.UTXO, error) {
	utxoIntf, err := vm.State.Get(db, utxoTypeID, ID)
	if err != nil {
		return nil, err
	}
	utxo, ok := utxoIntf.(*djtx.UTXO)
	if !ok {
...
func (vm *VM) putUTXO(db database.Database, utxo *djtx.UTXO) error {
	utxoID := utxo.InputID()
...
func (vm *VM) registerDBTypes() {
	...
	marshalUTXOFunc := func(utxoIntf interface{}) ([]byte, error) {
		if utxo, ok := utxoIntf.(*djtx.UTXO); ok {
			return Codec.Marshal(utxo)
		} else if utxo, ok := utxoIntf.(djtx.UTXO); ok {
			return Codec.Marshal(utxo)
		}
		return nil, fmt.Errorf("expected *djtx.UTXO but got type %T", utxoIntf)
	}
