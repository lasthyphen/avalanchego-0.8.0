...
// be created.
func (n *Node) initChains(genesisBytes []byte, djtxAssetID ids.ID) error {
...
func (n *Node) Initialize(Config *Config, logger logging.Logger, logFactory logging.Factory) error {
	...
	genesisBytes, djtxAssetID, err := genesis.Genesis(n.Config.NetworkID)
	if err != nil {
		return fmt.Errorf("couldn't create genesis bytes: %w", err)
	}
	if err := n.initChainManager(djtxAssetID); err != nil { // Set up the chain manager
	...
	if err := n.initChains(genesisBytes, djtxAssetID); err != nil { // Start the Platform chain
